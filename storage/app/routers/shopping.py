...
def clear_done():
    """Clear all completed items. Must be defined before {item_id} route."""
    conn = _get_db()
...
def add_shopping_item(body: ShoppingItemCreate):
    ...
    cur = conn.execute(
        "INSERT INTO shopping_list (product_id, amount, unit_id, note, recipe_id) VALUES (?, ?, ?, ?, ?)",
        (body.product_id, body.amount, body.unit_id, body.note, body.recipe_id),
    )
    conn.commit()
