...
## 0.17.1
- New `POST /api/stock/correct-purchase` endpoint for undoing an over-scan during a shopping session. Unlike `/stock/consume` (FIFO + logs a `consume` event), it removes stock newest-lot-first and reduces the matching recent `purchase` history events — so scanning 2 by mistake then dialing back to 1 reads as a clean net purchase of 1, with no phantom consumption in the history or waste/consume stats.

## 0.17.0
- Smarter shopping list: buying a *different brand of the same product type* now fulfils a list item. After a barcode shopping session, the new `POST /api/shopping-list/reconcile` endpoint asks the AI whether any item bought this trip is a variation (different brand or pack size, same type) of a remaining list item — e.g. you listed one Gouda but bought another, or any Béarnaise for a listed Béarnaise. It returns proposals only (never mutates); confirmed matches are applied via `POST /api/shopping-list/reconcile/apply`, which decrements the matched rows. Exact per-scan matching (`consume_shopping_for_purchase`) is unchanged and runs first; the AI pass only handles leftovers and never double-counts. Guards: confidence threshold (low-confidence dropped), at-most-once per row/bought item, hallucinated ids rejected, and graceful `ai_available: false` when the AI is offline so "finish" never fails.
- New per-item `pinned` flag on shopping list rows (default off). A pinned row wants that exact product and is excluded from cross-brand matching.
- Newly created ungrouped products (e.g. a brand-new brand scanned while shopping) are AI-placed under the correct type-parent in a background thread; skipped when no AI provider is configured, gated by the `autoplace_on_create` config key.

## 0.16.4
- Fix: partially restocking an auto-added shopping item no longer leaves it frozen at the original quantity. An auto-added row (created when stock drops below `min_stock_amount`) represents the remaining deficit, but `sync_auto_shopping` only ever inserted the full deficit or deleted the row once fully restocked — it never updated the amount in between. So buying 2 of a 3-needed item left the list still showing 3 (the `consume_shopping_for_purchase` decrement path deliberately skips auto-added rows). `sync_auto_shopping` now shrinks an existing auto-added row to the current deficit on every stock change, so the same partial purchase now reads 1. Manual rows are unchanged (still decremented on purchase); the row is still removed entirely once stock reaches the threshold.
...
## 0.15.0
- New `ha_storage.add_to_shopping_list_by_name` service for AI agents: resolves a free-text product name (Finnish or matched) to an existing Storage product with a deterministic case-insensitive match — exact match first, then substring — and adds it to the shopping list by reusing the standard `POST /api/shopping-list` path. Supports a response: returns `status: "added"` with the matched product, `"ambiguous"` with a candidates list when several products match, or `"not_found"` when nothing matches. The service never creates products; on `not_found` the caller is expected to chain `scraper.search_products` + `scraper.add_product` and retry. Fields: `name` (required), `amount` (default 1), `note` (optional). No schema or REST change — the resolution helper is a pure, separately unit-tested function in the integration.
